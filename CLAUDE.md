...
- `engine/` (main/render/loader/style, plus `walk.ts` for walkbox pathing/depth — pure and unit-tested) — the DOM/canvas layer: 320×180 pixel canvas, click-to-walk actor with waypoint routing, depth-scaled sprites, walk-behind props, floating speech over speakers (the log is a secondary transcript), Look/Talk/Interact/Combine verb bar (arming an inventory item with Interact gives "use X on Y"), dialogue options overlay, cutscenes, room-change fades, localStorage saves. SCUMM feel is the presentation bar — see `docs/2026-07-18-scumm-parity.md` for what's adopted vs deferred vs refused.
